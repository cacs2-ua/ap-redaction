...
def get_language(name: str) -> LanguageSpec:
    ...
    if name in ("anbn", "a^n b^n"):
        return get_language_anbn()
    if name in ("anbncn", "a^n b^n c^n"):
        return get_language_anbncn()
    if name in ("anbmbman", "anbmbman", "anbmbman", "anbmbman", "anbmbman"):
        # tolerate typos-ish; canonical below
        return get_language_anbmBmAn()
    if name in ("anbmbman", "anbmbman"):
        return get_language_anbmBmAn()
    if name in ("anbmbman",):
        return get_language_anbmBmAn()
    if name in ("anbmbman", "anbmbman"):
...
def make_batch(
    ...
    if language.name == "anbmBmAn":
        if not train:
            ...
            seqs = [language.generate_sequence(
...
class RNN(nn.Module):
    def __init__(
        ...
        self.lstms = nn.ModuleList(
            ...
            + [LSTMLayer(num_cells, num_cells, drop_prob=drop_prob, use_peepholes=use_peepholes) for _ in range(num_layers - 1)]
        )
...
def grid_search(
    ...
    *,
    ...
    if verbosity >= 1:
        print(f"[grid] lang={language.name} configs={len(configs)} seeds_per_config={len(seeds)} total_runs={len(configs)*len(seeds)}")
